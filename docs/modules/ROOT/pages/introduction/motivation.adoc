...
introduced its Capture-the-flag extension (which is discussed later in this book in context of the Multi Juicer platform in xref:part4/multi-juicer.adoc[Hosting platform for multiple users] and the dedicated chapter xref:part4/ctf.adoc#_hosting_a_ctf_event[Hosting a CTF event]).
