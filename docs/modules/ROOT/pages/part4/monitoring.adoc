...
. Start Grafana and visit it at http://localhost:3000
+
NOTE: Its default port is the same as Juice Shop's, so if you run both on the
same machine, one needs to be moved to a different port.
...
TIP: The "Juice Shop Instance Dashboard" template was forked from the
...
xref:part4/trainers.adoc#_trainers_guide[Trainer's guide].
